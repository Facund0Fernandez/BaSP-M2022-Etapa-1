...
var avengersArray = ['iron man', 'capitán américa', 'hulk', 'black widow', 'thor'];
for (var i = 0; i < avengersArray.length; i++) {
    alert(avengersArray[i]);
}
...
for (var i = 0; i < avengersArray.length; i++) {
    alert(avengersArray[i].substring(0,1).toUpperCase() + avengersArray[i].substring(1));
}
...
var sentence = '';
for (var i = 0; i < avengersArray.length; i++) {
    sentence = sentence + '  ' + avengersArray[i];
}
alert(sentence);
